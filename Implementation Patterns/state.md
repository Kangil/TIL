...
# 선언 타입
- 타입 이름에는 구현보다는 역할을 반영
- 변수 선언은 가급적 일반적인 타입으로 (ArrayList 보다는 Collection)
- 유연성을 위해 가급적 적은 정보만 (ArrayList 보다는 Collection) 소수의 대상에게 전달 

# 초기화
- 가급적 선언과 함께
- 초기화에 드는 비용이 큰 경우 생성과 초기화를 분리

# 열성적 초기화
- 변수가 선언 혹은 생성 직후 초기화 (선언문이나 생성자)

# 게으른 초기화
- 초기화 비용이 커서 성능에 영향이 있는 경우 사용
```java
...
```
- 컴퓨터 연산 능력이 떨어지는 과거에 많이 사용
- 초기 가동 시간이 중요한 경우에도 사용 (이클립스의 플러그인)
- 게으른 초기화를 사용하는 것은 해당 부분 성능이 중요하다는 것을 의미